...
*   load_data -> it reads the csv
*   create_input_output -> seperates the features and the target column
*   split_data -> splits the data into 80% training 20% testing
*   impute_median -> imputing the missing values from the person_income median with its median
*   binary_encode -> it binary encodes the columns person_gender and previous_loan_defualts_on_file
*   ordinal_encoding -> ordinal encodes the column person_education, encdoer from a pickle
*   one_hot_encdoing -> one hot encodes the inputted columns, encoder from a pickle
*   create_model -> defines the xgb model from a pickle
*   train_model -> fits the data to the model
*   evaluate_model -> makes the classificaiton report of the model
